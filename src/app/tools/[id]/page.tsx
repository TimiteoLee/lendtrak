...
export default function ToolDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [t, l] = await Promise.all([getTool(id), getLoansForTool(id)]);
      if (!t) {
        router.push("/tools");
        return;
      }
  ...
  return (
    <div>
      ...
      {editing ? (
        <form onSubmit={handleSave} className="mb-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={editName} onChange={(e) => setEditName(e.target.value)} className="input-field" required autoFocus />
            </div>
            <div>
              <label className="section-heading block mb-1">Description</label>
              <input type="text" value={editDescription} onChange={(e) => setEditDescription(e.target.value)} placeholder="Optional details..." className="input-field" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" value={editValue} onChange={(e) => setEditValue(e.target.value)} placeholder="0.00" min="0" step="0.01" className="input-field" />
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-3">
                {conditions.map((c) => (
                  <button key={c} type="button" onClick={() => setEditCondition(c)} className={editCondition === c ? "toggle-btn-active" : "toggle-btn"}>{c}</button>
                ))}
              </div>
            </div>
        ...
        <>
          <div className="flex items-start justify-between mb-1">
            ...
            <button onClick={startEditing} className="btn-secondary !py-1 text-xs flex items-center gap-1">
              <Pencil size={14} />
              Edit
            </button>
          </div>
          ...
          {tool.description && (
            <p className="text-sm mb-6" style={{ color: "var(--fg-muted)" }}>{tool.description}</p>
          )}
        </>
      )}
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        <span className="text-xs" style={{ color: "var(--fg-faint)" }}>{loans.length} record{loans.length !== 1 ? "s" : ""}</span>
      </div>

      {loans.length === 0 ? (
        <div className="text-center" style={{ border: "1px solid var(--border)", padding: "1.5rem", borderRadius: "var(--radius)" }}>
          <p className="italic text-sm" style={{ color: "var(--fg-faint)" }}>This tool has never been lent out.</p>
          <Link href="/lend" className="btn-primary inline-block mt-3 text-sm">Lend It</Link>
        </div>
      ) : (
        <div>
          {activeLoans.map((loan) => (
            <div key={loan.id} className="ledger-row py-3 px-2">
              <div className="flex items-center justify-between">
                <div>
                  <span className="font-medium">{loan.person?.name || "Unknown"}</span>
            ...
            <div key={loan.id} className="ledger-row py-3 px-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm" style={{ color: "var(--fg-faint)" }}>
                  {format(new Date(loan.borrowed_date), "MMM d")} &rarr; {format(new Date(loan.actual_return_date!), "MMM d, yyyy")}
                </span>
              </div>
              <div className="text-xs mt-1" style={{ color: "var(--fg-faint)" }}>
                Out: {loan.condition_out} &rarr; In: {loan.condition_in}
                {loan.fee_amount > 0 && (
                  <span> &middot; Fee: ${loan.fee_amount}{loan.fee_paid ? " (paid)" : " (unpaid)"}</span>
                )}
              </div>
            </div>
          ))}
